...
\item $11^{-1} \equiv x \mod 120 \implies 11x \equiv 1 \mod 120 \implies x = 11 + (120 \cdot z) \textrm{ where } x \in \mathbb{Z}$ \\

\item For a number $x$, if $\forall y \epsilon [1, x-1] : gcd(x,y) = 1$, then x must be prime. The extended Euclid algorithm gives us the multiplicative inverse of $a \mod n$ if and only if $gcd(a, n)=1$. This means that the only numbers in the set $\{0, 1, \ldots, x^m - 1\}$ that do not have inverses modulo $x^m$ are the numbers in the set $S = \{a_0, a_1, \ldots ,a_n\}$ such that the $gcd(a_i, x^m)=x$ where $0 \leq i \leq n$. The $gcd(a_i, x^m)=x$ because $x$ is prime. The cardinality of $S = \frac{x^m}{x} - 1$. Therefore, there are $x^m - (\frac{x^m}{x} - 1)$ numbers in the set $\{0, 1, \ldots, x^m - 1\}$ that have multiplicative inverses modulo $x^m$. Since finding the multiplicative inverse using the extended Euclid algorithm takes $O(n^3)$, the total running time takes $\boxed{O(n^3) \cdot [x^m - (\frac{x^m}{x} - 1)]}$.
