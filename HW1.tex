...
\item Let a tree with a single node have height 0 and define this row as row 0. This means that at row $k$, there will be at most $m^k$ nodes. Therefore, a tree of height $h$ will have at most $N$ nodes where \[N = \sum\limits_{k=0}^h m^k\ = \frac{m^{h+1} - 1}{m-1}.\] Solving for $h$ gives us the lower bound for the tree: \[\boxed{h = \lceil\log_m [N(m-1)+1]\rceil - 1}.\]
