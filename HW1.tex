...
\noindent \emph{Total:} $O(n^5)$. This was calculated by multiplying the big O times of the outer loop, the inner loop and the dominating modulat expoentiation, i.e. $O(n) \cdot O(n) \cdot O(n^5)$.
...
\item Using this definition of modular exponentiation, there will be $O(\log y) = O(n)$ recursive calls. 
...
\item $11^{-1} \equiv x \mod 120 \implies 11x \equiv 1 \mod 120 \implies x = 11 + (120 \cdot x) \textrm{ where } x \in \mathbb{Z}$ \\
$13^{-1} \equiv x \mod 45 \implies 13x \equiv 1 \mod 45 \implies x = 7 + (45 \cdot z) \textrm{ where } z \in \mathbb{Z}$ \\
$35^{-1} \equiv x \mod 77 \implies 35x \equiv 1 \mod 77 \implies$ x does not exist because $gcd(35,77) > 1$ \\
$9^{-1} \equiv x \mod 11 \implies 9x \equiv 1 \mod 11 \implies x = 5 + (11 \cdot z) \textrm{ where } z \in \mathbb{Z}$ \\
$11^{-1} \equiv x \mod 1111 \implies 11x \equiv 1 \mod 1111 \implies$ x does not exist because $gcd(11,1111) > 1$ \\
To find the modulo multiplicative inverse in the previous problems we used the extended Euclid algorithm. The algorithm can be demonstrated with $9^{-1} \equiv x \mod 11$ as follows:
...
According to the extending euclid algorithm 5 is the inverse of 9 $\mod 11$.
...
\item Part C is the result of the Chinese Remainder Theorem for $n = 2$. It is important to note for the solution that $C_x \equiv 1 \mod x$, $C_x \equiv 0 \mod y$, $C_y \equiv 0 \mod x$ and $C_y \equiv 1 \mod y$.

\vspace{10mm}

\[q \equiv m \mod x \implies q\cdot y\cdot(y^{-1} \mod x) \equiv m\cdot y\cdot(y^{-1} \mod x) \mod x\]
\[q \equiv n \mod y \implies q\cdot x\cdot(x^{-1} \mod y) \equiv n\cdot x\cdot(x^{-1} \mod y) \mod y\]
\[q \equiv m\cdot y\cdot(y^{-1} \mod x) \mod x + n\cdot x \cdot(x^{-1} \mod y) \mod y\]
\[q \equiv (m\cdot y\cdot(y^{-1} \mod x) + n\cdot x \cdot(x^{-1} \mod y)) \mod xy\]

\item The above equations and properties still hold for 3 primes. That is that $C_x \equiv 1 \mod x$, $C_x \equiv 0 \mod y$ and $C_x \equiv 0 \mod z$. The corresponding q can be computed as follows and is useful for number 9.
\[q = m\cdot(yz\cdot((yz)^{-1} \mod x)) + n\cdot(xz\cdot((xz)^{-1} \mod y))+o\cdot(xy\cdot((xy)^{-1} \mod z))\]
