...
\item Let a tree with a single node has height 0. A lower bound for the height is obtained when a row of nodes gets filled before the next level starts to get populated. A tree with the h-th row filled will have $N = m^h+1$ nodes if $h > 0$. This gives us a lower bound of $\boxed{h = \lceil \log _{m}(N-1)\rceil }$
